// landing4-bg.jsx — generative aurora background: drifting gradient blobs (low-res
// canvas upscaled = free blur), flowing noise-wave lines, scroll parallax.
// Renders into the #bg4 canvas. Exports startAurora4 to window.
(function () {
  // ---- tiny color helpers ----
  ...
  function paletteFrom(accentHex) {
    ...
    const mk = (dh, ds, dl) => hslToRgb([hsl[0] + dh, Math.min(1, Math.max(0.3, hsl[1] + ds)), Math.min(0.62, Math.max(0.3, hsl[2] + dl))]);
    return [
      mk(0, 0, 0),        // accent
      mk(0.10, 0, 0.04),  // neighbor hue, brighter
      mk(-0.12, -0.05, -0.02),
      mk(0.5, -0.2, -0.18), // dim complement, deep
    ];
  }
  ...
  // Singleton driver — restartable with new options when tweaks change.
  let state = null;
  function startAurora4(opts) {
    const { motion = 1, accent = '#4d84ff', waves = true } = opts || {};
    if (state) { state.kill(); state = null; }
    ...
    function drawBlobs(scroll) {
      ...
      for (const b of BLOBS) {
        ...
        g.addColorStop(0.55, `rgba(${cr},${cg},${cb},${b.alpha * 0.32})`);
    ...
    function drawWaves(scroll) {
      ...
      for (let i = 0; i < N; i++) {
        ...
        const a = 0.022 + 0.02 * Math.abs(Math.sin(i * 1.7 + t * 0.5));
    ...
    frame();
    // reduced motion: keep a static frame but still re-render on resize/scroll (cheap, throttled)
    let scrollTick = null;
    if (reduced) {
      const onScroll = () => {
        if (scrollTick) return;
        scrollTick = setTimeout(() => { scrollTick = null; frame(); }, 120);
      };
      window.addEventListener('scroll', onScroll, { passive: true });
      state = { kill: () => { alive = false; cancelAnimationFrame(raf); window.removeEventListener('resize', resize); window.removeEventListener('scroll', onScroll); } };
    } else {
      state = { kill: () => { alive = false; cancelAnimationFrame(raf); window.removeEventListener('resize', resize); } };
    }
  }

  Object.assign(window, { startAurora4 });
})();
