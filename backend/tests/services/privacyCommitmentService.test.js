import privacyCommitmentService from '../../src/services/privacyCommitmentService.js';
...
describe('PrivacyCommitmentService', () => {
    ...
    describe('Initialization', () => {
        it('should initialize successfully', () => {
            const info = privacyCommitmentService.getServiceInfo();
    ...
    describe('Balance Proofs', () => {
        ...
        it('should generate proof with exact threshold balance', async () => {
            const proof = await privacyCommitmentService.proveBalanceAboveThreshold('500', '500');
        ...
        it('should reject proof when balance is below threshold', async () => {
            await expect(
                privacyCommitmentService.proveBalanceAboveThreshold('100', '500')
        ...
        it('should reject negative threshold', async () => {
            await expect(
                privacyCommitmentService.proveBalanceAboveThreshold('1000', '-500')
            ).rejects.toThrow('Balance and threshold must be non-negative');
        });
    ...
    describe('Proof Verification', () => {
        ...
        it('should reject null proof', async () => {
            const isValid = await privacyCommitmentService.verifyBalanceProof(null);
            expect(isValid).toBe(false);
        });
    ...
    describe('Pedersen Commitments', () => {
        it('should create Pedersen commitment', async () => {
            const commitment = await privacyCommitmentService.createPedersenCommitment('1000');
        ...
        it('should create commitment with custom blinding factor', async () => {
            ...
            const commitment = await privacyCommitmentService.createPedersenCommitment('1000', customBlinding);
        ...
        it('should create different commitments for same amount (random blinding)', async () => {
            const commitment1 = await privacyCommitmentService.createPedersenCommitment('1000');
            const commitment2 = await privacyCommitmentService.createPedersenCommitment('1000');
        ...
        it('should reject negative amount', async () => {
            await expect(
                privacyCommitmentService.createPedersenCommitment('-1000')
            ).rejects.toThrow('Amount must be non-negative');
        });

        it('should handle zero amount', async () => {
            const commitment = await privacyCommitmentService.createPedersenCommitment('0');
        ...
        it('should handle large amounts', async () => {
            ...
            const commitment = await privacyCommitmentService.createPedersenCommitment(largeAmount);
    ...
    describe('Poseidon Hash', () => {
        it('should hash single input', async () => {
            const hash = await privacyCommitmentService.hashPoseidon(['123']);
        ...
        it('should hash multiple inputs', async () => {
            const hash = await privacyCommitmentService.hashPoseidon(['123', '456', '789']);

            expect(hash).toBeTruthy();
        });

        it('should produce consistent hashes for same inputs', async () => {
            const hash1 = await privacyCommitmentService.hashPoseidon(['123', '456']);
            const hash2 = await privacyCommitmentService.hashPoseidon(['123', '456']);

            expect(hash1).toBe(hash2);
        });

        it('should produce different hashes for different inputs', async () => {
            const hash1 = await privacyCommitmentService.hashPoseidon(['123']);
            const hash2 = await privacyCommitmentService.hashPoseidon(['456']);

            expect(hash1).not.toBe(hash2);
        });

        it('should handle BigInt inputs', async () => {
            const hash = await privacyCommitmentService.hashPoseidon([BigInt(123), BigInt(456)]);
        ...
        it('should handle empty array', async () => {
            await expect(
                privacyCommitmentService.hashPoseidon([])
            ).rejects.toThrow();
        });
    });
    ...
    describe('Error Handling', () => {
        it('should throw error if service not initialized', async () => {
            const uninitializedService = Object.create(Object.getPrototypeOf(privacyCommitmentService));
        ...
        it('should handle invalid input types gracefully', async () => {
            await expect(
                privacyCommitmentService.proveBalanceAboveThreshold('invalid', '500')
            ).rejects.toThrow();
        });

        it('should handle null inputs', async () => {
            await expect(
                privacyCommitmentService.proveBalanceAboveThreshold(null, '500')
            ).rejects.toThrow();
        });
    });
    ...
    describe('Performance', () => {
        it('should generate proof in reasonable time (<2 seconds)', async () => {
            ...
            await privacyCommitmentService.proveBalanceAboveThreshold('1000', '500');
        ...
        it('should verify proof in reasonable time (<500ms)', async () => {
            ...
            const startTime = Date.now();
            await privacyCommitmentService.verifyBalanceProof(proof);
            const endTime = Date.now();
        ...
        it('should create commitment in reasonable time (<100ms)', async () => {
            const startTime = Date.now();
            await privacyCommitmentService.createPedersenCommitment('1000');
            const endTime = Date.now();
